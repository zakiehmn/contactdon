...
class Contactdon:
    ...
    def __init__(self, contactDic = {}, contactFile = 'contactdict.json',
                 groupFile = 'groupsList.json', groups = []):
    ...
    def add(self, paramsDict):
        ...
        if(len(paramsDict.keys()) == 4):
            if(self.check_unique_fnamelname(paramsDict["first_name"], paramsDict["last_name"]) and 
               check_correctPhoneNum(paramsDict["phone_number"]) and 
               check_correctEmail(paramsDict["email_address"]) ):
    ...
    def check_unique_fnamelname(self, fname, lname):
        ...
        if(n>0):
            for contact in self.contactDic.values():
                if(contact._fName == fname and contact._lName == lname):
                    return False
        return True
        ...
        for key in paramsDict.keys():
            ...
            if(key == "email_address"):
                if(self.check_unique_emailaddress(paramsDict["email_address"]) == False or
                   check_correctEmail(paramsDict["email_address"]) == False):
                    check = False
            if(key == "phone_number"):
                if(self.check_unique_phonenum(paramsDict["phone_number"]) == False or
                   check_correctPhoneNum(paramsDict["phone_number"]) == False):
                    check = False
        return check
    ...
    def check_unique_fname(self, id, fname):
        for contact in self.contactDic.values():
            if(fname == contact._fName and contact._lName == contact._lName):
                return False
        return True
        
    def check_unique_lname(self, id, lname):
        for contact in self.contactDic.values():
            if(lname == contact._lName and 
               self.contactDic[id]._fName == contact._fName ):
                return False
        return True

    def check_unique_phonenum(self, phonenumber):
        for contact in self.contactDic.values():
            if(contact._phoneNumber == phonenumber):
                return False
        return True
       
    def check_unique_emailaddress(self, emailaddress):
        for contact in self.contactDic.values():
            if(contact._emailAddress == emailaddress):
                return False
        return True
